...
export class MetadataEngine {
    ...
    roleDetector = new BusinessRoleDetector();
    displayGenerator = new DisplayNameGenerator();
    aggregationDetector = new AggregationDetector();
    execute(dataset) {
        return dataset.headers.map(header => {
            const values = dataset.rows.map(row => row[header]);
            const detectedType = this.typeDetector.execute(values);
            const businessRole = this.roleDetector.execute(header, detectedType);
            return {
                originalName: header,
                displayName: this.displayGenerator.execute(header),
                detectedType,
                businessRole,
                aggregation: this.aggregationDetector.execute(header, detectedType, businessRole),
                visible: true,
                confidence: 95,
                sampleValues: values
